...
| `kramits` | 3,880 | **Not importable — stimuli unobtainable (2026-09-01)** | The OSF project (osf.io/xmkzs) holds one file, `mitsumasu_data_code_rev.zip`: the eLife *analysis* bundle (RData, HDDM CSVs, R scripts). Its readme defines `leftItem`/`rightItem` only as "the item ID#"; no images, no stimulus list. The lab's OSU 147-snack numbering does not transfer: by raw index, kramits and `smikrab2018` item means correlate r = −0.20 (permutation null 95th pct +0.13), and offset ±1, reversed, and `gwikrab`'s `picNum` order all fall inside the null. Construct and scale are settled from the paper ("how much they would like to eat the food", 0–10; space bar for foods they did not want). Note the ratings reconstruct from choice trials: 3,880 of 6,660 subject×item cells (58%), and the absences are the space-bar rejections and never-paired items, so they skew toward disliked foods. The Zurich experiment's stimulus folder is not available, so the 148 items cannot be named; the dataset stays out. |

The kramits check exposed a second listing-order error already in the database — six datasets named from zip order rather than the experiments' alphabetical numbering — corrected in migration 029 (docs/DATASET_AUDIT.md, section G).
